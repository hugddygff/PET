...
class SignModel(nn.Module):
    ...
    def forward(
        ...
        encoder_output, encoder_hidden = self.encode(
            sgn=sgn, sgn_mask=sgn_mask, local_mask=local_mask, sgn_length=sgn_lengths
        )
    ...
    def encode(
        self, sgn: Tensor, sgn_mask: Tensor, local_mask: Tensor, sgn_length: Tensor
    ) -> (Tensor, Tensor):
        """
    ...
    def decode(
        ...
        return self.decoder(
            ...
            trg_pos=self.pos_embed(x=pos_input, mask=txt_mask),
            trg_mask=txt_mask,
            unroll_steps=unroll_steps,
    ...
    def get_loss_for_batch(
        ...
        if self.do_translation:
            ...
            word_outputs, _, pos_out, _, _ = decoder_outputs
            # Calculate Translation Loss
            txt_log_probs = F.log_softmax(word_outputs, dim=-1)

            pos_log_probs = F.log_softmax(pos_out, dim=-1)
            ...
            pos_loss = (
                    translation_loss_function(pos_log_probs, batch.pos)
                    * 0.3
            )

            translation_loss += pos_loss
    ...
    def run_batch(
        ...
        encoder_output, encoder_hidden = self.encode(
            sgn=batch.sgn, sgn_mask=batch.sgn_mask, local_mask=batch.local_mask, sgn_length=batch.sgn_lengths
        )
...
def build_model(
    ...
    pos_padding_idx = pos_vocab.stoi[PAD_TOKEN]
    ...
    if do_translation:
        ...
        pos_embed: Union[Embeddings, None] = Embeddings(
            **cfg["decoder"]["embeddings"],
            num_heads=cfg["decoder"]["num_heads"],
