...
class TransformerDecoder(Decoder):
    ...
    def __init__(
        ...
        **kwargs
        ...
        #self.last_layer = TransformerDecoder(size=hidden_size,ff_size=ff_size,num_heads=num_heads,dropout=dropout)

        self.pe = PositionalEncoding(hidden_size)
        self.layer_norm = nn.LayerNorm(hidden_size, eps=1e-6)

        #self.mid_layer = nn.Linear(300, hidden_size, bias=False)

        self.emb_dropout = nn.Dropout(p=emb_dropout)
        self.output_layer = nn.Linear(hidden_size, vocab_size, bias=False)

        self.output_pos = nn.Linear(hidden_size, pos_size, bias=False)

        self.pos_embed = pos_embed

        #self.mem1 = nn.Linear(hidden_size, 1)
        #self.mem2 = nn.Linear(2*hidden_size, 1)
        self.mem3 = nn.Linear(hidden_size, vocab_size, bias=False)

        self.mem3.weight.data.copy_(torch.from_numpy(np.load("full_embed.npy")[:,:512]))
        ...
        # self.contexts = torch.from_numpy(np.load("full_embed.npy")).float().cuda()
        # self.contexts = nn.Parameter(torch.Tensor(vocab_size, hidden_size * 2).cuda())
        # x_p = self.mem1(x)                # B * T * d
        # c_p = self.mem2(self.contexts)      # V * d
    ...
    def forward(
        ...
        **kwargs
        ...
        #trg_embed = self.mid_layer(trg_embed)
        ...
        pos_out = self.output_pos(self.layer_norm(x))

        #add the pos embedding
        #x = x + 0.0 * trg_pos

        x = self.layers[-1](x=x, memory=encoder_output, src_mask=src_mask, trg_mask=trg_mask)
        x = self.layer_norm(x)
        ...
        #return output, x, pos_out, None, None
        return xc, x, pos_out, None, None
        #return xc + output, x, pos_out, None, None
    ...
    def infer(
        ...
        **kwargs
        ...
        pos_idx = torch.argmax(pos_out, -1)

        trg_pos = self.pos_embed(pos_idx)
